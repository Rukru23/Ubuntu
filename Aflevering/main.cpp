...
int main()
{
   ...
   //Test lodtrækning uden tilbage lægning
   Bin b;
   ...
   std::cout<<"Draw: "; for(unsigned int i= 0; i < 10; ++i){ std::cout<<b.drawAndRemove()<<""; } std::cout<<std::endl;
   ...
   std::cout<<"Draw: "; for(unsigned int i = 0; i < 10; ++i){ std::cout<<b.drawAndRemove()<<""; } std::cout<<std::endl;
   std::cout<<"Binsize (after): "<<b.size()<<std::endl; std::cout<<std::endl;
...
/*
   ...
   //Test lod trækning af faglighed ved en eksamen
   ...
   for(Student& s:students){ std::cout<<std::left<<std::setw(22)<<s.getName()<<" "<<s.getUsername()<<std::endl; }

   std::cout<<std::endl;
   //Lodtrækning uden tilbage lægning
   Bin bCourse;
   bCourse.fillBin(0,exams1.size()-1,students.size());

   for(Student&s:students){ s.draw(bCourse); }

   //Udskriv resultat af lodtrækning for hver studerende
   std::cout<<"Studerende og brugernavne med lodtrækning"<<std::endl;
   for(Student&s:students){ std::string examDraw = s.getExam(exams1);
       std::cout<<std::left<<std::setw(22)<<s.getName()<<" "<<std::setw(10)<<s.getUsername()<<" "<< examDraw <<std::endl; }
   std::cout<<std::endl<<"Anden lodtrækning"<<std::endl;

   //Anden lodtrækning
   bCourse.fillBin(0,exams2.size()-1,students.size());
   for(Student&s:students){ s.draw(bCourse); }
   for(Student&s:students){ std::string examDraw = s.getExam(exams2);
       std::cout<<std::left<<std::setw(22)<<s.getName()<<""<<std::setw(10)<<s.getUsername()<<""<<examDraw<<std::endl; }
*/
    return 0;
}
